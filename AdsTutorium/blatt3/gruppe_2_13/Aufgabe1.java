...
public class Aufgabe1 {
	...
	public static int linearSuche1(int wert,int[]array){
		...
		for(zaehler=0;zaehler<array.length;zaehler++){
			if(wert==array[zaehler])//save in index the Position where the Value is egals to the required value 
				index=zaehler;
		}
		return index;
	}
	...
	public static int linearSuche2(int wert,int[]array){
		...
		for(zaehler=0;zaehler<array.length;zaehler++){
			if(wert==array[zaehler]){//zaehler is at the position of the first element equals to the one we are looking for
				return zaehler;
			}
		}
	...
	public static int iterativBinaereSuche(int wert,int[]array){
		...
		while(weiter){
			...
			if(array[index]==wert){//The value was found and the location is returned
				return index;
			}
	...
	public static void main(String[] args) {
		...
		println("Linearsuche(letzte Treffer): index= "+ausgabe);
