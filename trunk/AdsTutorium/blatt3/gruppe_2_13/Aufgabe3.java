...
public class Aufgabe3 {
	...
	public static int[] selectionSort(int[] array) {
		...
		while (anfangIndex <= endIndex) {
			// in that case, the smaller element between the first and the last
			// square is in the first square
			...
			// if a higher value was not found and the current higher value is
			// not in the last square
			if (!groesserGefunden && groesste != endIndex) {
				stelleTauchen(array, groesste, endIndex);
				vertauchungenAnzahl++;
				// if the smaller value found was at this location, it is now
				// where the higher value was
				if (kleinste == endIndex)
					kleinste = groesste;
			}
			// if a smaller value was not found and the current smaller value is
			// not in the first square
			if (!kleinerGefunden && kleinste != anfangIndex) {
				stelleTauchen(array, kleinste, anfangIndex);
				vertauchungenAnzahl++;
				// if the higher value found was at this location, it is now
				// where the smaller value was
