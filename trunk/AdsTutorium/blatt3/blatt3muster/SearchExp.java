...
/**
 * @author Peter Kaiser
 * modified by Wolfgang Schramm
 * @version 1.1
 * Implementierung von 4 Such-Algorithmen in einem Array
 * Lineare Suche (bis zum ersten Erfolg): seekLin2
 * Lineare Suche im ganzen Array: seekLin1
 * Binäre Suche iterativ: seekBinItr
 * Binäre Suche rekursiv: seekBinRec
 * Zur Zählung der Vergleiche
 * (c) 2009 Peter Kaiser
 * ADS Übung WS2011; 1IB
 * Blatt 3 / Aufgabe 4
 * HINWEIS: Die Suchverfahren sind dergestalt modifiziert, dass sie nicht mehr den Index des gefundenen
 * Elements zurückgeben, sondern die Anzahl der durchgeführten Vergleiche.
 * Der Bequemlichkeit halber wurden for-Schleifen verwendet, obwohl die noch nicht in GDI eingeführt wurden.
 */
public class SearchExp {
	...
	public static void main(String[] args)  {
		...
		for (int i=0; i<data1.length; i++) {
			data1[i] = i*2;  // there is always one number in between 2 entries, e.g. 1, 3, 5, 7, ...
		};
		...
		for (int i=0; i<data2.length; i++) {
			data2[i] = i*3; // there are always two numbers in between 2 entries, e.g. 1, 4, 7, 10, ...
		};
		...
		for (int i=0; i<data3.length; i++) {
			data3[i] = i*3; // there are always three numbers in between 2 entries, e.g. 1, 5, 9, 13, ...
		};
	...
	/**
	 * seekLin1
	 * linear search, searching the complete array
	 * @param data	array to be search
	...
	/**
	 * seekLin2
	 * linear search, searching till found
	 * @param data	array to be search
	 * @param key	search key
	 * @return		number of comparions
	 */
	...
	/**
	 * seekBR
	 * binary search, recursive, internal used fct with additional parameters
	 * @param data	array to be search
	 * @param left	leftmost index of search array
	 * @param rght	rightmost index of search array
	 * @param key	search key
	...
	static public int seekBR (int [] data, int left, int rght, int key) {	
		...
		if (left <= rght) {
			...
			cnt++;
			if (data [middle] == key) return cnt;	// key found
			else {
				cnt++;
				if (key > data [middle]) {
